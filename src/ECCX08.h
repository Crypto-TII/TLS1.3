...
public:
  ...
  int generateSharedKey(int slot, byte publicKey[], byte sharedKey[]);  // M.Scott 12/7/2021
  ...
  int beginSHA256();
  int updateSHA256(const byte data[],int len); // variable
...
private:
  ...
  int receiveResponse(void* response);

...
void create_private(octad *PRIVATE,octad *RAWPRIVATE) {
    ...
    if (PKTYPE==RSA_PK)
    {
        ...
        OCT_append_octad(&NUMBERS,&ZERO);
/*
        makeclause(INT,len,(unsigned char *)&RAWPRIVATE->val[1*len],&PARAM);
        OCT_append_octad(&NUMBERS,&PARAM);
        makeclause(INT,len,(unsigned char *)&RAWPRIVATE->val[0*len],&PARAM);
        OCT_append_octad(&NUMBERS,&PARAM);
        makeclause(INT,len,(unsigned char *)&RAWPRIVATE->val[3*len],&PARAM);
        OCT_append_octad(&NUMBERS,&PARAM);
        makeclause(INT,len,(unsigned char *)&RAWPRIVATE->val[2*len],&PARAM);
        OCT_append_octad(&NUMBERS,&PARAM);
        makeclause(INT,len,(unsigned char *)&RAWPRIVATE->val[4*len],&PARAM);
        OCT_append_octad(&NUMBERS,&PARAM);
*/
        for (int j=0;j<5;j++)
        {
